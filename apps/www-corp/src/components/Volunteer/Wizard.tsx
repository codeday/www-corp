import React, {
  useState, useReducer, useEffect, RefObject
} from 'react';
...
export default function Wizard({ events, formRef, startBackground='', startRegion='', startPage=0, startSelection=false, after }: WizardProps) {
        ...
        <Box>
          ...
          <Box m={6}>
            <Collapse in={commitmentLevel >= 0} animateOpacity>
                <Text>
                  <Checkbox onChange={(e: any) => setCommitmentLevel(e.target.checked? 1 : 0)}><b>I am interested in organizing a CodeDay in my city</b></Checkbox>
                </Text>
              <br/>
            </Collapse>
            <Collapse in={commitmentLevel >= 1} animateOpacity>
              <Text>
                ...
                <TextInput value={region} onChange={(e: any) => { setRegion(e.target.value); setHasSelection(true) }} w="md" display="block"/>
              </Text>
              <br/>
            </Collapse>
          </Box>
  ...
  const pageEmail = (
    <Box>
      ...
      <VStack w="md" mb={3}>
          <TextInput m={1} placeholder="First Name" value={firstName} onChange={(e: any) => setFirstName(e.target.value)}/>
          ...
          { background === 'industry'? <TextInput m={1} placeholder="LinkedIn" value={linkedin} onChange={(e: any) => { setLinkedin(e.target.value) }} /> : undefined }
      </VStack>
  ...
  const [page, navigate] = useReducer((prev: number, action: string) => Math.max(0, (action === 'next' ? prev + 1 : prev - 1), (action === 'last' ? pages.length - 2 : 0)), resolvedStartPage);
  const isFinalPage = page === pages.length - 1;

  const [hasStarted, setHasStarted] = useState(false);
  useAfterMountEffect(() => {
    if (!hasStarted) {
      (global as any).analytics?.track('volunteer.started', { style: 'full' });
    }
    setHasStarted(true);
  ...
  useAfterMountEffect(() => {
    if(isFinalPage) {
      (global as any).analytics?.track('volunteer.submitted');
      linkedInConversion('volunteer.submitted');
      if (email) (global as any).analytics?.identify(email);
    } else {
      (global as any).analytics?.track('volunteer.partial', {
