...
export default function Home({ seed }: { seed: number }) {
  ...
  const justVolunteers = shuffle(volunteers.filter((v: any) => !otherIds.includes(v.id)), seed);
  const uniqueBoard = board.filter((director: any) => !employeeIds.includes(director.id));

  const boardEmeritusNames = boardEmeritus.map((e: any) => toTitleCase(e.givenName + ' ' + e.familyName).replace(/( \.| \*)/g, ''));
  const emeritusNames = emeritus.map((e: any) => toTitleCase(e.givenName + ' ' + e.familyName).replace(/( \.| \*)/g, ''));
