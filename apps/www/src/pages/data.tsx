  ...
  return (
    <Page slug={`/data`} title="Open Datasets">
      <Content mt={-8} mb={2}>
        ...
        <Box mb={4}>
          <Link fontSize="sm"{...{href:`https://doi.org/${process.env.NEXT_PUBLIC_DOI_PREFIX}/${process.env.NEXT_PUBLIC_DATABASE_DOI}`} as any}>
        ...
        <List listStyleType="disc" pl={4}>
            {cms.publications.items.map((p: any) => (
                <ListItem key={p.doiSuffix}>
                    <Box as="a"{...{href:`/doi/${process.env.NEXT_PUBLIC_DOI_PREFIX}/${p.doiSuffix}`} as any}>
