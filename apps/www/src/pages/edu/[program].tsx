...
function Faq({ faq }: FaqProps) {
  ...
  return (
    <Box borderBottomWidth={1}>
      <Heading as="h4" fontSize="lg" onClick={() => setIsOpen(!isOpen)} cursor="pointer" p={4}>
        {faq.title}
      </Heading>
      {isOpen && (
        <Box p={4}>
...
export default function EducationProgram({ seed }: EducationProgramProps) {
  const { colorMode } = useColorMode();
  const { cms } = useQuery();
  const { isFallback, query } = useRouter();
  const program = cms?.eduPrograms?.items[0];
  const testimonial = shuffle(JSON.parse(JSON.stringify(cms?.testimonials?.items || [])), seed).filter(
    (e: any) => !e.groupName,
  )[0] as any;
        ...
        <Grid templateColumns={{ base: '1fr', md: '1fr 1fr', lg: '6fr 3fr' }} gap={8}>
          <Box>
            ...
            {testimonial && (
              <Box borderLeftWidth={2} pl={4} ml={2} mb={8}>
                ...
                <Text>
                  &mdash;{testimonial.firstName} {testimonial.lastName}, {testimonial.title || testimonial.type}{' '}
                  {testimonial.company}
                </Text>
              </Box>
            )}
          ...
          <Box>
            ...
            {program.presentingSponsors?.items?.length > 0 && (
              <Box mt={4} textAlign="center">
                <Heading as="h3" fontSize="lg" color="current.textLight" mb={4}>
                  Presented in partnership with
                </Heading>
                {program.presentingSponsors.items.map((sponsor: any) => (
                  <Link href={sponsor.link} rel="noopener" target="_blank" key={sponsor.name}>
          ...
          <Box>
            <Box borderWidth={2} borderColor="green.600" rounded="sm">
              <Heading as="h3" fontSize="2xl" mb={2} bg="green.600" p={4} color="white">
                Request More Info
              </Heading>
              <Box p={4}>
            ...
            {faqs && faqs.length > 0 && (
              <>
                <Heading as="h3" fontSize="2xl" mt={8} mb={2}>
                  FAQs
                </Heading>
...
export const getStaticPaths: GetStaticPaths = async () => {
  const query = await apiFetch(print(ProgramQuery), {}, {});

  return {
    paths:
      query?.cms?.programs?.items?.map((i: any) => ({
        params: {
          program: i.webname,
        },
      })) || [],
