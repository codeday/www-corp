...
function DuckPrint({x, y, r, d}: DuckPrintProps) {
  ...
  return (
        <img
          ...
          onAnimationEnd={(e: React.AnimationEvent<HTMLImageElement>) => (e.currentTarget.style.animation = '')}
          src="/footprint.svg" />
    )
}
...
function makePath(startX: number, startY: number, startR: number, numSteps: number): Step[] {
  let x = startX
  let y = startY
  let r = startR
  let bias = 0
...
interface DocumentSizeEm {
  width: number | undefined;
  height: number | undefined;
}

export default function DuckPrints({ stepDelay=1500 }: { stepDelay?: number }) {
  const PATH_CHUNKS = 10
  const prefersReducedMotion = usePrefersReducedMotion()
  const [path, setPath] = useState<Step[]>(makePath(Math.random() * 100 + 1000, -2, Math.random() * 45, PATH_CHUNKS))
  // initial path has a large offset because wrap does not work with negative numbers
  const [documentSizeEm, setDocumentSizeEm] = useState<DocumentSizeEm>({width: undefined, height: undefined})
  useEffect(() => {
  ...
  if(prefersReducedMotion) {
    ...
    return (
        <Box position="absolute" w="100%" h="100%" overflow="clip">
          ...
          {path.map((p, idx) => {
            return (<DuckPrint x={p.x % ((documentSizeEm.width ?? 0) * 1.1)} y={p.y % ((documentSizeEm.height ?? 0) * 1.1)} r={p.r} d={(idx) /1.5} />)
          })}
        </Box>
    )
}
