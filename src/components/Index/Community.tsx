...
export default function Community({ seed, ...props }: { seed?: any; [key: string]: any }) {
  ...
  const cards: ReactElement[] = shuffle([
    ...
    ...(shuffle(indexCommunityPhotos.items, seed)
      .map((p: any) => <Card key={p.photo.url} photo={p.photo.url} eventInfo={{ region: p.region, event: p.event }} />)
        || []
    ).slice(0, 25),
    ...(shuffle(showcase.photos, seed)
      .map((p: any) => <Card key={p.url} photo={p.url} eventInfo={{ region: p.region, program: p.program }} />)
        || []
    ),
  ], seed);
  const rows = [
