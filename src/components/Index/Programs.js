      ...
      <Content>
        ...
        <Grid templateColumns={{ base: '1fr', md: '1fr 1fr' }} gap={8}>
          <Box borderWidth={1} borderColor="current.border" borderRadius={2} p={4} boxShadow="md">
            ...
            {JSON.stringify(nextUpcomingEvent(codeDay?.linkedFrom?.events?.items)) != "null" && Math.floor((nextUpcomingEvent(codeDay?.linkedFrom?.events?.items).startsAt - new Date()) / (24 * 60 * 60 * 1000)) <= 30 &&
            ...
            {mainPrograms?.items?.map((program) => (
              ...
              >
                <Box mb={1}>
                  ...
                  {JSON.stringify(nextUpcomingEvent(program.linkedFrom?.events?.items)) != "null" && Math.floor((nextUpcomingEvent(program.linkedFrom?.events?.items).startsAt - new Date()) / (24 * 60 * 60 * 1000)) <= 30 &&
                    <Image src="/soon.svg" height={10} alt="" float="right" />}
