...
export default function Quotes({ seed }: QuotesProps) {
  ...
  const textQuotes = shuffle(quoteTestimonials?.items.filter((q: any) => !q.video), seed);
  const videoQuotes = shuffle(quoteTestimonials?.items.filter((q: any) => q.video?.url && q.image?.url), seed);
