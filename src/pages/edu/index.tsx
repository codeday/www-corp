    ...
    <Page slug="/edu" title="Education">
      <Content>
        ...
        <Grid templateColumns={{ base: '1fr', md: 'repeat(2, 1fr)', lg: 'repeat(3, 1fr)' }} gap={4}>
          {eduPrograms?.items?.filter((program: any) => program.educationDetails).map((program: any) => (
        ...
        <Grid templateColumns={{ base: '1fr', md: 'repeat(2, 1fr)', lg: 'repeat(3, 1fr)' }} gap={4}>
          {communityPartners?.items?.map((partner: any) => (
            <IconBox as="a" display="block" href={partner.url} target="_blank" rel="noopener" h="100%" key={partner.url}>
              ...
              <BoxBody>
                ...
                {partner.regions?.items?.length > 0 && (
                  <>
                    <br /><br />({partner.regions.items.map((r: any) => r.name).join(', ')})
                  </>
                )}
              </BoxBody>
            </IconBox>
